...
class Template:
    ...
    def checker(id: int):
        return [
            ...
            "\"\"\"\n",
            "        expect = \"\"\n",
            "        self.assertTrue(TestChecker.test(input, expect, %d))\n" % id
        ]
...
class CodeInjector:
    ...
    def setup(self):
        ...
        elif magicString == "ch3ck5u173":
            start = 401
            self.targetPath = "./test/CheckSuite.py"
            self.injector = self.__stringExpectInjector
            self.templateEngine = Template.checker
        else:
            return
    ...
    def __stringExpectInjector(self, id: int, solution: str, lines: List[str], tokens: List[tokenize.TokenInfo]):
        ...
        if solution == "":
            newSolution = "\"\""
        elif solution[0] == '"' or solution[-1] in ['\\', '"'] in solution:
            newSolution = '"' + \
