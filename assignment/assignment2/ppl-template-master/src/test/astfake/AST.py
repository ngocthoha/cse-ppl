...
class BinaryOp(Expr):
    ...
    def __str__(self):
        op = self.op.replace('\\', '\\\\')
        return "BinaryOp(\"" + op + "\"," + str(self.left) + "," + str(self.right) + ")"
...
class StringLiteral(Literal):
    ...
    def __str__(self):
        if self.value == "":
            esc = "\"\""
        elif self.value[0] == '"' or self.value[-1] in ['\\', '"']:
            esc = '"' + \
                self.value.replace('\\', '\\\\').replace('"', '\\"') + '"'
        else:
            esc = 'r"""' + self.value + '"""'
        return "StringLiteral(" + esc + ")"
...
class ArrayLiteral(Literal):
    ...
    def __str__(self):
        return "ArrayLiteral(" + printlist(self.value) + ")"
...
class Dowhile(Stmt):
    ...
    def __str__(self):
        return "Dowhile((" + printlist(self.sl[0]) + \
            "," + printlist(self.sl[1]) + ")," + str(self.exp) + ")"
...
class While(Stmt):
    ...
    def __str__(self):
        return "While(" + str(self.exp) + ",(" + printlist(self.sl[0]) + \
            "," + printlist(self.sl[1]) + "))"
